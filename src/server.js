...
module.exports = async (client) => {
        ...
        if (banned) {
            client.users.fetch(req.user.id).then(async a => {
                client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
                    ...
                    dynamic: true
                })).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://discordbotlist-1.mesifytr.repl.co/user/${a.id}) ID'li kullanıcı giriş yaptı.`).addField("Kullanıcı adı", a.username).addField("Kullanıcı ID", a.id).addField("Kullanıcı Ayrımcılığı", a.discriminator))
            })
            req.session.destroy(() => {
                res.json({
                    login: false,
                    message: "LuxiList'ten engellendiniz.",
                    logout: true
                })
                req.logout();
            });
    ...
    app.get("/code/:code", checkMaintence, checkAuth, async (req, res) => {
        ...
        if (!koddata) return res.redirect('/codes?error=true&message=Sanırım yanlış koda girdin.')
        if (!client.guilds.cache.get(config.server.id).members.cache.get(req.user.id)) return res.redirect("/error?code=403&message=İlk önce discord sunucumuza katılmalısın.");
        if (koddata.codeCategory == "javascript") {
            if (!client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(config.server.roles.codeshare.javascript)) return res.redirect("/error?code=403&message=Bu kodu görmeye iznin yok.");
        }
        if (koddata.codeCategory == "html") {
            if (!client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(config.server.roles.codeshare.html)) return res.redirect("/error?code=403&message=Bu kodu görmeye iznin yok.");
        }
        if (koddata.codeCategory == "subs") {
            if (!client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(config.server.roles.codeshare.altyapilar)) return res.redirect("/error?code=403&message=Bu kodu görmeye iznin yok.");
        }
        if (koddata.codeCategory == "5invites") {
            if (!client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(config.server.roles.codeshare.besdavet)) return res.redirect("/error?code=403&message=Bu kodu görmeye iznin yok.");
        }
        if (koddata.codeCategory == "10invites") {
            if (!client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(config.server.roles.codeshare.ondavet)) return res.redirect("/error?code=403&message=Bu kodu görmeye iznin yok.");
        }
        if (koddata.codeCategory == "15invites") {
            if (!client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(config.server.roles.codeshare.onbesdavet)) return res.redirect("/error?code=403&message=Bu kodu görmeye iznin yok.");
        }
        if (koddata.codeCategory == "20invites") {
            if (!client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(config.server.roles.codeshare.yirmidavet)) return res.redirect("/error?code=403&message=Bu kodu görmeye iznin yok.");
        }
        if (koddata.codeCategory == "bdfd") {
            if (!client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(rconfig.server.roles.codeshare.bdfd)) return res.redirect("/error?code=403&message=Bu kodu görmeye iznin yok.");
        }
    ...
    app.post("/uptime/add", checkMaintence, checkAuth, async (req, res) => {
        ...
        if (!rBody['link']) {
            res.redirect('?error=true&message=Lütfen bir bağlantı ekle.')
        } else {
            if (!rBody['link'].match('https')) return res.redirect('?error=true&message=Doğru bir bağlantı gibi görünmüyor.')
            ...
            if (dd.length > 9) res.redirect('?error=true&message=Uptime limitini aştın.')

            if (dde) return res.redirect('?error=true&message=Bu bağlantı zaten sistemde var.')
            client.users.fetch(req.user.id).then(a => {
                client.channels.cache.get(channels.uptimelog).send(new Discord.MessageEmbed()
                    ...
                    .setDescription("Sisteme yeni bir bağlantı katıldı.")
            ...
            res.redirect('?success=true&message=Bağlantınız başarıyla uptime sistemimize eklendi.');
        }
    })
    ...
    app.get("/addbot", checkMaintence, checkAuth, async (req, res) => {
        if (!client.guilds.cache.get(config.server.id).members.cache.get(req.user.id)) return res.redirect("/error?code=403&message=Bunu yapmak için ilk önce discord sunucumuza katılmalısın.");
    ...
    app.get("/bot/:botID/vote", checkMaintence, async (req, res) => {
        ...
        if (!botdata) return res.redirect("/error?code=404&message=Sanırım yanlış Bot ID'si girdin.");
        if (req.user) {
            if (!req.user.id === botdata.ownerID || req.user.id.includes(botdata.coowners)) {
                if (botdata.status != "Approved") return res.redirect("/error?code=404&message=Sanırım yanlış Bot ID'si girdin.");
            }
        }
    ...
    app.post("/bot/:botID/vote", checkMaintence, checkAuth, async (req, res) => {
        ...
        if (x) return res.redirect("/error?code=400&message=12 saatte bir oylama yapabilirsin.");
        ...
        return res.redirect(`/bot/${req.params.botID}/vote?success=true&message=Başarıyla oyladınız. 12 Saat sonra tekrar oy verebilirsiniz.`);
        renderTemplate(res, req, "botlist/vote.ejs", {
            req,
            roles,
            config,
    ...
    app.post("/addbot", checkMaintence, checkAuth, async (req, res) => {
        ...
        if (botvarmi) return res.redirect('/error?code=404&message=Eklemeye çalıştığınız bot sistemde bulunmaktadır.');

        client.users.fetch(req.body.botID).then(async a => {
            if (!a.bot) return res.redirect("/error?code=404&message=Sanırım yanlış Bot ID'si girdin.");
            if (!a) return res.redirect("/error?code=404&message=Sanırım yanlış Bot ID'si girdin.");
            if (rBody['coowners']) {
    ...
    app.get("/bot/:botID/edit", checkMaintence, checkAuth, async (req, res) => {
        ...
        if (req.user.id == botdata.ownerID || botdata.coowners.includes(req.user.id)) {
            ...
            res.redirect("/error?code=404&message=TBu botu düzenlemek için sahiplerinden biri olmalısınız.");
        }
    });
    ...
    app.get("/bot/:botID/delete", async (req, res) => {
        ...
        if (req.user.id === botdata.ownerID || botdata.coowners.includes(req.user.id)) {
            ...
            return res.redirect("/error?code=404&message=Sanırım yanlış Bot ID'si girdin.");
        }
    })
    ...
    app.post("/certification/apply", checkMaintence, checkAuth, async (req, res) => {
        ...
        res.redirect("/bots?success=true&message=Bot onaylandı.&botID=" + rBody['bot'])
        let botdata = await botsdata.findOne({
            botID: rBody['bot']
        })
        client.channels.cache.get(channels.botlog).send(`**${req.user.username}** adlı kullanıcı **${botdata.username}** adlı bota doğrulama isteği gönderdi.`)
    });
    //
    const checkAdmin = async (req, res, next) => {
        if (req.isAuthenticated()) {
            if (client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(roles.yonetici) || client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(roles.moderator)) {
                ...
                res.redirect("/error?code=403&message=Bunu yapmaya iznin yok.")
            }
        } else {
            req.session.backURL = req.url;
            res.redirect("/login");
    ...
    app.get("/admin/confirm/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(req.params.botID).then(bota => {
            client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}> adlı kullanıcının **${bota.tag}** adlı botu kabul edildi. `)
            client.users.cache.get(botdata.ownerID).send(`Hey! **${bota.tag}** adlı botun yetkililer tarafından kabul edildi ve listeye girdin.`)
        });
        ...
        return res.redirect(`/admin/unapproved?success=true&message=Bot kabul edildi.`)
    });
    // DELETE BOT
    ...
    app.post("/admin/decline/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(botdata.ownerID).then(sahip => {
            ...
            client.users.cache.get(botdata.ownerID).send(`**${botdata.username}** adlı botun maalesef reddedildi.\nSebep: **${rBody['reason']}**\nYetkilendirilen: **${req.user.username}**`)
        })
        await botsdata.deleteOne({
            botID: req.params.botID,
            ownerID: botdata.ownerID
    ...
    app.get("/admin/certificate/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(botdata.botID).then(bota => {
            client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}> adlı kullanıcının **${bota.tag}** adlı botu onaylandı ve kullanıcıya onaylı geliştirici verildi.`)
            client.users.cache.get(botdata.ownerID).send(`Hey! **${bota.tag}** adlı botun başarıyla onaylandı. Ayrıca **Onaylı Geliştirici** rozet ve rolünü almaya hak kazandın!`)
        });
    ...
    app.post("/admin/certificate/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(botdata.botID).then(bota => {
            client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}> adlı kullanıcının **${bota.tag}** adlı botu onaylanmadı.`)
            client.users.cache.get(botdata.ownerID).send(`**${bota.tag}** adlı botunuz onaylanamadı.\nSebep: **${rBody['reason']}**`)
        });
        await appsdata.deleteOne({
            botID: req.params.botID
        })
        ...
        return res.redirect(`/admin/certificate-apps?success=true&message=Onay silindi.`)
    });
    ...
    app.post("/admin/addcode", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
            .setTitle("Yeni bir kod eklendi!").setColor("GREEN").setFooter(config.footer)
            .setDescription(`**[${req.user.username}](https://discordbotlist-1.mesifytr.repl.co/user/${req.user.id})** adlı kullanıcı **${rBody['codename']}** adında bir kodu sisteme ekledi.`)
            .addField("Kod Bağlantısı", `https://discordbotlist-1.mesifytr.repl.co/code/${kod}`, true)
    ...
    app.get("/admin/editcode/:code", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        if (!koddata) return res.redirect('/codes?error=true&message=Sanırım yanlış kod girdin.')
    ...
    app.post("/admin/editcode/:code", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
            ...
            .setDescription(`**[${req.user.username}](https://discordbotlist-1.mesifytr.repl.co/user/${req.user.id})** adlı kullanıcı **${rBody['codename']}** adında bir kodu düzenledi.`)
            .addField("Kod Bağlantısı", `https://discordbotlist-1.mesifytr.repl.co/code/${kod}`, true)
            .addField("Kod Açıklaması", rBody['codedesc'], true)
            .addField("Kod Kategorisi", rBody['category'], true)
        )
    ...
    app.get("/admin/deleteuptime/:code", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        return res.redirect('../admin/uptimes?error=true&message=Bağlantı silindi.');
    });
    ...
    app.post("/admin/maintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        if (bakimdata) return res.redirect('../admin/maintence?error=true&message=Bakım modu zaten açık.');
        client.channels.cache.get(channels.webstatus).send(`:tools: LuxiList, **${req.body.reason}** sebebiyle __bakıma alındı.__`).then(a => {
            new maintenceSchema({
                server: config.server.id,
        ...
        return res.redirect('../admin/maintence?success=true&message=Bakım açıldı.');
    });
    app.post("/admin/unmaintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        const bakimsonaerdikardesTopic = new dc.MessageEmbed()
            .setAuthor("LuxiList", client.user.avatarURL())
            .setThumbnail(client.user.avatarURL())
            .setColor("GREEN")
            .setDescription(`:green_circle: Sitemiz şimdi tekrardan **aktif!**\n[Siteye Giriş Yap](https://discordbotlist-1.mesifytr.repl.co)`)
            .setFooter("Luzixus © All rights reserved.");
        await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => {
            a.edit(`~~ :tools: LuxiList, **${bakimdata.reason}** sebebiyle bakıma alındı! ~~`, bakimsonaerdikardesTopic)
        })
    ...
    app.post("/admin/userunban", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        return res.redirect('../admin/userban?success=true&message=Kullanıcıının Yasağı Kaldırıldı.');
    });
    ...
    app.post("/admin/partners", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        return res.redirect('/admin/partners?success=true&message=Partner erklendi.')
    });
    //---------------- ADMIN ---------------\\
    ...
    app.post("/user/:userID/edit", checkMaintence, checkAuth, async (req, res) => {
        ...
        return res.redirect('?success=true&message=Profilin başarıyla düzenlendi.');
    });
    //------------------- PROFILE -------------------//
    ...
    app.get("/api/bots/:botID", async (req, res) => {
        ...
        if (!botinfo) return res.json({
            "error": "Sanırım yanlış Bot ID'si girdin."
        })
        res.json({
